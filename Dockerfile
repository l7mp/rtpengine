...
RUN apt-get update \
  ...
  && cd /usr/local/src \
  && git clone -b feature-restore-calls-on-ksadd https://github.com/l7mp/rtpengine.git \
  && cd rtpengine/daemon \
  && make && make install \
...
COPY ./etc/rtpengine.sample.conf /etc/rtpengine.conf

#ENTRYPOINT ["/entrypoint.sh"]

CMD ["rtpengine", "--config-file", "/etc/rtpengine.conf"]
